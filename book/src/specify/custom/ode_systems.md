...
So far we've focused on using custom structs to specify individual equations, but you can also use custom structs to specify the entire system of equations. This is useful if you have
a large amount of shared data between the equations.
...
you have already implemented these traits for your structs. Similar to the `ParameterisedOp` struct you have seen in the previous sections, these will all be parameterised by the lifetime
of the underlying struct that implements `OdeEquations`.
