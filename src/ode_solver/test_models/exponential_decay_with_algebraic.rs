...
#[allow(unused_mut)]
fn exponential_decay_with_algebraic<M: DenseMatrix>(x: &M::V, p: &M::V, _t: M::T, mut y: &mut M::V) 
{
    y.copy_from(x);
    y.mul_assign(-p[0]);
    let nstates = y.len();
...
#[allow(unused_mut)]
fn exponential_decay_with_algebraic_jacobian<M: DenseMatrix>(_x: &M::V, p: &M::V, _t: M::T, v: &M::V, mut y: &mut M::V) {
    y.copy_from(v);
    y.mul_assign(-p[0]);
    let nstates = y.len();
...
fn exponential_decay_with_algebraic_mass<M: DenseMatrix>(x: &M::V, _p: &M::V, _t: M::T, y: &mut M::V) {
    y.copy_from(x);
    let nstates = y.len();
