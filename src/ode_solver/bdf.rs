use crate::{Scalar, vector::Vector, nonlinear_solver::{NonLinearSolver, self}, IndexType, callable::{ode::BdfCallable, Callable}};



pub struct Bdf<T, V, C, NLS> 
where
    T: Scalar,
    V: Vector<T>,
    C: Callable<T, V>,
    NLS: NonLinearSolver<T, V, C>,
{
    t: T,
    nlinsol: NLS,
    order: IndexType,
    ...
    NLS: NonLinearSolver<T, V, C>,
{
    ...
    const MIN_FACTOR: T = T::from(0.2).unwrap();
    const MAX_FACTOR: T = T::from(10.0).unwrap();

    fn new(callable: C, y0: V, p: V, t0: T, h0: T, mass: M) -> Self {
        let nstates = callable.nstates();
        let order = 1;
        let nonlinear_solver = NLS::new(callable);
        let h = Self::select_initial_step(atol, rtol, callable, t0, y0.clone(), f0, h0);
        let n_equal_steps = 0;
        let diff = V::zeros(nstates);
        let kappa = T::from(1.0).unwrap();
        let gamma = T::from(1.0).unwrap();
        let alpha = T::from(1.0).unwrap();
        let c = T::from(1.0).unwrap();
        let error_const = T::from(1.0).unwrap();
        let psi = T::from(1.0).unwrap();
