...
impl<T: Scalar> Matrix for Mat<T> {
    ...
    fn split_at_indices(&self, indices: &<Self::V as Vector>::Index) -> (Self, Self, Self, Self) {
        ...
        let not_indices = (0..n).filter(|i| !indices.contains(i)).collect::<Vec<_>>();
        let ni = indices.len();
        let nni = not_indices.len();
        let mut ul = Self::zeros(nni, nni);
        for &i in not_indices.iter() {
            for &j in not_indices.iter() {
                ul[(i, j)] = self[(i, j)];
            }
        }
        let mut ur = Self::zeros(nni, ni);
        for &i in not_indices.iter() {
            for &j in indices.iter() {
                ur[(i, j - nni)] = self[(i, j)];
            }
        }
        let mut ll = Self::zeros(ni, nni);
        for &i in indices.iter() {
            for &j in not_indices.iter() {
                ll[(i - nni, j)] = self[(i, j)];
            }
        }
        let mut lr = Self::zeros(ni, ni);
        for &i in indices.iter() {
            for &j in indices.iter() {
                lr[(i - nni, j - nni)] = self[(i, j)];
            }
        }
        (ul, ur, ll, lr)
    }
    ...
    fn combine_at_indices(ul: &Self, ur: &Self, ll: &Self, lr: &Self, indices: &<Self::V as Vector>::Index) -> Self {
        ...
        let not_indices = (0..n).filter(|i| !indices.contains(i)).collect::<Vec<_>>();
        let mut ret = Self::zeros(ul.nrows() + ll.nrows(), ul.ncols() + ur.ncols());
        for i in 0..ul.nrows() {
            for j in 0..ul.ncols() {
                ret[(i, j)] = ul[(i, j)];
            }
        ...
        for i in 0..ur.nrows() {
            for j in 0..ur.ncols() {
                ret[(i, j + ul.ncols())] = ur[(i, j)];
            }
        }
        for i in 0..ll.nrows() {
            for j in 0..ll.ncols() {
                ret[(i + ul.nrows(), j)] = ll[(i, j)];
            }
        }
        for i in 0..lr.nrows() {
            for j in 0..lr.ncols() {
                ret[(i + ul.nrows(), j + ul.ncols())] = lr[(i, j)];
            }
        }
        ret
    }
