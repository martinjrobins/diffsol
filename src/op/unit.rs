...
impl<'a, M: Matrix> LinearOp for ParametrisedOp<'a, UnitCallable<M>> {
    fn gemv_inplace(&self, x: &Self::V, _t: Self::T, beta: Self::T, y: &mut Self::V) {
        y.axpy(Self::T::one(), x, beta);
    }
}

impl<'a, M: Matrix> NonLinearOp for ParametrisedOp<'a, UnitCallable<M>> {
    fn call_inplace(&self, x: &Self::V, _t: Self::T, y: &mut Self::V) {
        y.copy_from(x);
    }
}

impl<'a, M: Matrix> NonLinearOpJacobian for ParametrisedOp<'a, UnitCallable<M>> {
    fn jac_mul_inplace(&self, _x: &Self::V, _t: Self::T, v: &Self::V, y: &mut Self::V) {
        y.copy_from(v);
    }
}

impl<'a, M: Matrix> NonLinearOpAdjoint for ParametrisedOp<'a, UnitCallable<M>> {
    fn jac_transpose_mul_inplace(&self, _x: &Self::V, _t: Self::T, v: &Self::V, y: &mut Self::V) {
        y.copy_from(v);
    }
}

impl<'a, M: Matrix> NonLinearOpSens for ParametrisedOp<'a, UnitCallable<M>> {
    fn sens_mul_inplace(&self, _x: &Self::V, _t: Self::T, _v: &Self::V, y: &mut Self::V) {
        y.fill(Self::T::zero());
    }
}

impl<'a, M: Matrix> NonLinearOpSensAdjoint for ParametrisedOp<'a, UnitCallable<M>> {
    fn sens_transpose_mul_inplace(&self, _x: &Self::V, _t: Self::T, _v: &Self::V, y: &mut Self::V) {
        y.fill(Self::T::zero());
    }
}

impl<'a, M: Matrix> LinearOpSens for ParametrisedOp<'a, UnitCallable<M>> {
    fn sens_mul_inplace(&self, _x: &Self::V, _t: Self::T, _v: &Self::V, y: &mut Self::V) {
        y.fill(Self::T::zero());
    }
}

impl<'a, M: Matrix> LinearOpTranspose for ParametrisedOp<'a, UnitCallable<M>> {
