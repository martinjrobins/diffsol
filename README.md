...
- `cuda`: Use in-built C CUDA linear algebra containers and solvers (disabled by default).
- `cuda-oxide`: Use in-built [cuda-oxide](https://github.com/NVlabs/cuda-oxide) linear algebra containers and solvers (disabled by default). This needs the cuda-oxide toolchain: `cargo oxide` on the pinned nightly. See the `oxide-test` recipe in the `Justfile`.
...
- `autodiff`: Uses [`std:autodiff`](https://rustc-dev-guide.rust-lang.org/autodiff/internals.html), enables `*_autodiff` methods on `OdeBuilder` to calculate gradients automatically.
