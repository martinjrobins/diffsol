...
int FUNC_NAME(void)
{
  ...
  if (check_retval((void *)A, "SUNSparseMtarix", 0))
    return (1);

  /* Create KLU SUNLinearSolver object */
  LS = SUNLinSol_KLU(uu, A, ctx);
  if (check_retval((void *)LS, "SUNLinSol_KLU", 0))
    return (1);
...
int heatres(realtype, N_Vector uu, N_Vector up, N_Vector resval,
            void *user_data)
{
...
int jacHeat3(realtype, realtype cj,
...
int jacHeat(realtype, realtype cj,
            N_Vector, N_Vector, N_Vector,
            SUNMatrix JJ, void *,
            N_Vector, N_Vector, N_Vector)
{
  ...
  for (i = TOTAL - 2 * (MGRID - 2) - 1; i < TOTAL - 2; i += 2)
    data[i] = -ONE / (dx * dx);
  for (i = TOTAL - 2 * (MGRID - 2); i < TOTAL - 1; i += 2)
    data[i] = ONE;
  data[TOTAL - 1] = ONE;
  ...
  /* middle row values */
  for (i = 0; i < (MGRID - 4); i++)
    rowvals[TOTAL - 6 * (MGRID - 2) + 1 + 4 * i] = MGRID * MGRID - 1 - 2 * (MGRID - 1) - MGRID + 1 + i;
  for (i = 0; i < (MGRID - 4); i++)
    rowvals[TOTAL - 6 * (MGRID - 2) + 2 + 4 * i] = MGRID * MGRID - 1 - 2 * (MGRID - 1) + i;
  for (i = 0; i < (MGRID - 4); i++)
    rowvals[TOTAL - 6 * (MGRID - 2) + 3 + 4 * i] = MGRID * MGRID - 1 - 2 * (MGRID - 1) + 1 + i; /*copied above*/
  for (i = 0; i < (MGRID - 4); i++)
...
static void PrintHeader(realtype, realtype)
{
#if 0
...
static void PrintOutput(void *, realtype, N_Vector)
{
#if 0
  int retval;
  realtype umax, hused;
