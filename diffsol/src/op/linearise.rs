...
mod tests {
    ...
    impl NonLinearOpJacobian for FakeJacOp {
        fn jac_mul_inplace(&self, x: &Self::V, _t: Self::T, v: &Self::V, y: &mut Self::V) {
            y.copy_from(&Self::V::from_vec(
                vec![
                    x.get_index(0, 0) * v.get_index(0, 0) + v.get_index(1, 0),
                    2.0 * v.get_index(0, 0) + x.get_index(1, 0) * v.get_index(1, 0),
                ],
    ...
    fn linearised_op_tracks_state_and_uses_jacobian_helpers() {
        ...
        assert_eq!(matrix.get_index(0, 0, 0), 3.0);
        assert_eq!(matrix.get_index(1, 0, 0), 2.0);
        assert_eq!(matrix.get_index(0, 1, 0), 1.0);
