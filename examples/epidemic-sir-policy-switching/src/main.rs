...
fn main() {
    ...
    let susceptible: Vec<_> = soln.ys.inner()[0].row(0).into_iter().copied().collect();
    let infected: Vec<_> = soln.ys.inner()[0].row(1).into_iter().copied().collect();
    let recovered: Vec<_> = soln.ys.inner()[0].row(2).into_iter().copied().collect();
